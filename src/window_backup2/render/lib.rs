...
impl Backend {
    ...
    pub fn render<T>(&mut self, desc: RenderDescriptor, build_gui: &mut T)
    where
        T: BuildUI
    {
        ...
        let full_output = self.ctx.run(raw_input, move |ctx| {
            build_gui.build_ui(ctx);
        });
        self.platform
